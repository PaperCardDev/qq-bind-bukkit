...
//import cn.paper_card.qq_bind.api.QqBot;
...
////        final QqBot bot = botOnline ? (qq, message) -> logger.fine("发送群At消息 {qq: %d, msg: %s}".formatted(qq, message)) : null;
////
////        { // 删除绑定
////            final BindInfo info = qqBindApi.getBindService().queryByUuid(uuid);
////            if (info != null) {
////                final boolean deleted = qqBindApi.getBindService().removeBind(info.uuid(), info.qq());
...
////        System.out.println();
////
////        final List<String> names = qqBindApi.getBindCodeService().queryPlayerNames();
////        Assert.assertTrue(names.contains("Paper99"));
////
////        Assert.assertTrue(response.bindCode() > 0);
////
////        {
////            final QqBindApi.BindCodeInfo info = qqBindApi.getBindCodeService().takeByCode(response.bindCode());
...
////
////        { // 删除绑定
////            final QqBindApi.BindInfo info = qqBindApi.getBindService().queryByUuid(uuid);
////            if (info != null) {
////                final boolean deleted = qqBindApi.getBindService().deleteByUuidAndQq(info.uuid(), info.qq());
////                Assert.assertTrue(deleted);
////            }
////        }
////
...
////
////        { // 确保这个QQ没有被绑定
////            final QqBindApi.BindInfo info = qqBindApi.getBindService().queryByQq(testQq);
////            if (info != null) {
////                final boolean deleted = qqBindApi.getBindService().deleteByUuidAndQq(info.uuid(), info.qq());
////                Assert.assertTrue(deleted);
////            }
...
////        final long testQQ = 1398872296;
////
////        assert qqBindApi.getBindCodeService().getCodeCount() > 0;
////
////        // 模拟发送验证码
